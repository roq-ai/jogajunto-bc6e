...
import { CourtListPage as ShowCaseList } from 'pages/courts';

function DashboardPage() {
  const { session } = useSession();

  const groupStat = useSWR<PaginatedInterface<GroupInterface>>(
    () => `/groups`,
    () => getGroups({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Groups',
      result: groupStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`courts`} passHref legacyBehavior>
                  <Button
